...
<#
.SYNOPSIS
    kanjo CLI installer for Windows
.DESCRIPTION
    Downloads and installs the latest kanjo CLI from GitHub Releases.
    Verifies SHA-256 checksum. Installs to ~/AppData/Local/kanjo-cli/.
.EXAMPLE
...
$Repo = 'belulu-dev/kanjo-cli'
$BinaryName = 'kanjo'
$InstallDir = if ($env:KANJO_INSTALL_DIR) { $env:KANJO_INSTALL_DIR } else { Join-Path $env:LOCALAPPDATA 'kanjo-cli' }
...
try {
    ...
    Write-Host "Installing kanjo v$version (windows/$arch)..."

    $tmpDir = Join-Path ([System.IO.Path]::GetTempPath()) "kanjo-install-$(Get-Random)"
    ...
    try {
        ...
        Write-Host ''
        Write-Host "Done! kanjo v$version installed to $dst"
        Write-Host "Restart your terminal, then run 'kanjo version' to verify."
    }
    finally {
